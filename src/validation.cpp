                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        BOOST_FOREACH(const CTxMemPool::txiter it, allConflicting)
                    {
                        LogPrint("mempool", "replacing tx %s with %s for %s MIR additional fees, %d delta bytes\n",
...
/*    if(nHeight <= 6765 && nHeight > 1)
        nSubsidy = 1 * COIN;
    if(nHeight <= 12333 && nHeight > 6765)
        nSubsidy = 1 * COIN;
    if(nHeight <= 17711 && nHeight > 12333)
        nSubsidy = 2 * COIN;
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Mircoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
